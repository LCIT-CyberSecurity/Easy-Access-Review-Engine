...
export async function getPage(path: string, params: Params = {}): Promise<Page> { const body = await getJson(path, params) as Partial<Page>; return { items: body.items ?? [], total: Number(body.total ?? 0), limit: Number(body.limit ?? 25), offset: Number(body.offset ?? 0) } }
...
export function currentStateLabels(observed: boolean, expected: boolean): string[] { return ['Observed ' + (observed ? '✓' : 'Not observed'), 'Expected ' + (expected ? '✓' : 'Not expected')] }
